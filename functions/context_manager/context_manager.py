...
class Filter:
    ...
    async def _is_code_only_message(self, content: str) -> bool:
        ...
        Uses a fast path for large raw code pastes based on Python
        structural line ratio and CrossEncoder intent check on the prose.
        """
        ...
        if estimated_tokens >= self.valves.lean_user_code_min_tokens:
            ...
            # 1. Extract only the non‑code prose
            ...
            # 3. Cheap guard: explicit question mark
            ...
            # 4. CrossEncoder on prose_text only
            if self._cross_encoder:
                ...
                if has_intent:
                    ...
                    return False
                # No intent → treat as code-only
                code_ratio = sum(1 for l in non_blank if _PY_STRUCTURAL.match(l)) / len(
                    non_blank
                )
                return code_ratio > 0.07
            else:
                # CrossEncoder not available – log and do not treat as silent
                self._log_debug(
                    "_is_code_only_message: CrossEncoder not loaded, "
                    "cannot detect intent – falling back to non‑silent."
                )
                return False
        # ── End fast path ────────────────────────────────────────────────
    ...
    async def _db_worker(self):
        ...
        while True:
            try:
                ...
                self._log_debug("DB worker cancelled — shutting down.")
                break
            except Exception as e:
                self._log_debug(f"DB worker crashed: {e} — restarting in 2s")
                await asyncio.sleep(2)  # Pequeña pausa para no hacer busy-loop

    async def _db_worker_loop(self):
    ...
    async def _detect_contradictions(self, messages: List[dict]) -> Optional[str]:
        ...
        # CrossEncoder not available – log and skip detection
        self._log_debug(
            ...
            for i, doc in enumerate(results["documents"][0]):
                ...
                if sim >= self.valves.duplicate_question_threshold and doc != query:
                    ...
                    if self._cross_encoder:
                        ...
                        # ce_score[0] is similarity; threshold can be a valve
    ...
    async def _should_keep_full_code(self, user_question: str) -> bool:
        ...
        # CrossEncoder not available – log and keep full code as safe default
        self._log_debug(

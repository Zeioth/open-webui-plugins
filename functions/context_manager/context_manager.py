...
class Filter:
    ...
    class Valves(BaseModel):
        ...
        multi_call_chunk_max_tokens: int = Field(
            default=28000,
            description="Max tokens per chunk sent to the analysis model.",
        )
        multi_call_model: str = Field(
            default="Qwen2.5-Coder-7B-Instruct-Q4_K_M",
            description="Fast model used to analyze each code chunk.",
        )
        ...
        multi_call_cache_enabled: bool = Field(
            default=True,
            description="Cache chunk analyses per block+question.",
        )
        multi_call_inject_suggested: bool = Field(
            default=True,
            description="Inject the full code of symbols suggested by the chunks.",
        )
        multi_call_suggested_max_tokens: int = Field(
            default=3000,
            description="Token budget for injecting suggested symbol code.",
        )
        multi_call_synthesis_model: str = Field(
            default="Qwen2.5-Coder-7B-Instruct-Q4_K_M",
            description="Model used to synthesize the final summary.",
        )
    ...
    async def _retrieve_all_memories_unified(
        ...
        try:
            ...
            if self.valves.enable_reranking and self._cross_encoder and docs_with_meta:
                rerank_k = min(
                    self.valves.reranker_top_k or self.valves.long_term_memory_top_k, 50
                )
    ...
    async def inlet(self, body: dict, __user__: Optional[dict] = None) -> dict:
        ...
        # Load symbol analysis cache from DB for this project (once per project life)
        ...
        # Process deferred secondary tasks at the start of each inlet
        if self.valves.defer_secondary_tasks:
    ...
    def _chunk_code_blocks_with_dependencies(
        ...
        for chunk_blocks in final_chunks:
            ...
            # ---- 2. Code blocks (truncated if too large) ----
    ...
    # --------------------------------------------------------------------------
    # Symbol-level analysis cache (memory + SQLite)
    # --------------------------------------------------------------------------
    ...
    async def _analyze_code_via_symbols(
        ...
        # Gather symbols with clean contexts, storing block hash for cache
        symbol_contexts = []  # (name, ctx_or_cached_dict, block_hash)
        seen_symbols = set()
        for block in state["active_blocks"].values():
            ...
            for sym in block.symbols:
                ...
                # Check cache (memory + DB) with content hash for invalidation
        ...
        for idx in fresh_indices:
            ...
            for attempt in range(max_retries):
                ...
                if parsed:
                    ...
                    # Store with content hash for future invalidation
